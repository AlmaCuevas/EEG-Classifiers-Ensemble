...
def call_tcanet(dataset_name, subject_id, train_loader, valid_loader, test_loader, channels, only_global_model: bool = True):  # only use global model
    ...
    if dataset_name == 'torres':
        ...
        kernel_size = 6
    elif 'coretto' in dataset_name:
        kernel_size = 2
    elif 'nieto' in dataset_name:
        kernel_size = 4
...
if __name__ == '__main__':
    ...
    train_loader, valid_loader, test_loader = EEGdata_loader(data, label)
    model_global, model_local, model_top = call_tcanet(dataset_name, subject_id, train_loader, valid_loader, test_loader, dataset_info['#_channels'], True)

    model_global, model_local, model_top = call_tcanet(dataset_name, subject_id, train_loader, valid_loader, test_loader, dataset_info['#_channels'], False)