...
# It simply doesn't work. To "predict" takes the mean values of X already knowing Y.
# I tried:
#       * giving X and y of train, but also X_test. (so it can do the means on the train but use them on X_test)
#       * giving X and a fake y (for example a 0 or a 1) (to see how the Y affected the ‘prediction’)
#       * giving fake values of the mean of X (0.5, 0.5) or (0.5, -0.5)

# If the prediction is by knowing the mean of X at least the version of having the mean of the train should work.
# Simply this method looks like pure cheating.
# The accuracies are fake, if you need to give them the answer for them to know the answer it is a scam.
...
def evaluate_online(model, state, dataloader, train_inputs, train_labels):
    ...
    with torch.no_grad():
        ...
        predicted_labels = model.pred_online(train_inputs, train_labels, test_inputs).to(device)
...
if __name__ == '__main__':
    ...
    for i in range(1,dataset_info['subjects']):
        ...
        train_inputs, train_labels = torch.tensor(dataloader.dataset.data_train).float().to(device), torch.tensor(dataloader.dataset.labels_train).to(device)
        pred_list = []
        for data_chosen, trial_chosen in zip(data_test, labels_test):
            ...
            dataloader = DataLoader(dataset, batch_size=5300,
                                    num_workers=workers, pin_memory=True,
                                    shuffle=True)
            answer = evaluate_online(net,best_model_state, dataloader, train_inputs, train_labels)
