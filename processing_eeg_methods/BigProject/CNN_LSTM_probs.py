...
def CNN_LSTM_train(data, labels, num_classes: int):
    ...
    X_train, X_test, X_val, y_train, y_test, y_val = train_test_val_split(
        dataX=data, dataY=labels, valid_flag=True
    )
    ...
    X_train = X_train.reshape(N_train, int(T_train / sampling), sampling, C_train)[
        :, :, 0, :
    ]
    X_val = X_val.reshape(N_val, int(T_val / sampling), sampling, C_val)[:, :, 0, :]
    X_test = X_test.reshape(N_test, int(T_test / sampling), sampling, C_test)[
        :, :, 0, :
    ]
    ...
    # add 1-layer cnn
    model.add(Conv1D(40, kernel_size=20, strides=4, input_shape=(timesteps, data_dim)))
...
def CNN_LSTM_test(model, trial_data):
    ...
    sampling = 1
    trial_data = trial_data.reshape(N_test, int(T_test / sampling), sampling, C_test)[
        :, :, 0, :
    ]
