...
def group_methods_train(
    ...
    if methods["customized"]:
        ...
        models_outputs["customized_clf"], models_outputs["customized_accuracy"] = customized_train(copy.deepcopy(data), labels)
...
if __name__ == "__main__":
    ...
    for subject_id in range(
        ...
        # SAVE DATAFRAME
        temp = pd.DataFrame({'Methods': activated_methods, 'Subject ID': [subject_id] * len(activated_methods), 'Version': [version_name] * len(activated_methods), 'Train Accuracy': train_final_accuracy.values(), 'Train Timer': train_final_timer.values(), 'Test Accuracy': test_final_accuracy.values(), 'Test Timer': test_final_timer.values()}) # mean_accuracy_per_subject
