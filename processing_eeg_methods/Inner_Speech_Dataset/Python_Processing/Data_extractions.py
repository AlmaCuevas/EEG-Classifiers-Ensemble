...
def Extract_subject_from_BDF(root_dir, N_S, N_B):
    ...
    file_name = (
        ...
        + "_ses-0"
        + str(N_B)
        + "_task-innerspeech_eeg.bdf"
    )
    rawdata = mne.io.read_raw_bdf(
...
def Extract_data_multisubject(root_dir, N_S_list, datatype="EEG"):
    ...
    for N_S in N_S_list:
        ...
        for N_B in N_B_arr:
            ...
            if datatype == "eeg":
                ...
                if (
                    S == 0 and N_B == 1
                ):  # assume same number of channels, time steps, and column labels in every subject and session
    ...
    for i in range(total_elem):
        ...
        if datatype == "eeg" or datatype == "exg":
            Y[offset : offset + rows[i], :] = tmp_list_Y[
                0
            ]  # only build Y for the datatypes that uses it
        offset += rows[i]
        del tmp_list_X[0]
        del tmp_list_Y[0]
        gc.collect()
