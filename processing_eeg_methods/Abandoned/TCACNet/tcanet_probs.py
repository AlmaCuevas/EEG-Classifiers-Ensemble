...
from TCACNet.tcacnet_base import (CustomDataset, EEGdata_loader, call_tcanet,
                                  tcanet_online_pred)
...
def tcanet_train(dataset_name, subject_id, data, labels, channels, ONLY_GLOBAL_MODEL):
    ...
    model_global, model_local, model_top, acc = call_tcanet(
        ...
        test_loader,
        channels=channels,
        only_global_model=ONLY_GLOBAL_MODEL,
    )  # There are 2 models here, use False to access the second version
    if acc <= 0.25:
        acc = np.nan
...
def tcanet_test(
    ...
    output_array = tcanet_online_pred(
        model_global,
        model_local,
        model_top,
        loader,
...
if __name__ == "__main__":
    ...
    print("******************************** Training ********************************")
    start = time.time()
    model_global, model_local, model_top, acc = tcanet_train(
        dataset_name,
        subject_id,
        data_train,
        labels_train,
