...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        sample = (self.data[index]).reshape(1, 16, 701)
...
if __name__ == "__main__":
    # Manual Inputs
    subject_id = 5  # Only two things I should be able to change
    ...
    for ep in range(n_epochs):
        ...
        valid_total_loss, valid_cross_entropy, valid_acc = tcanet_net(
            model_global,
            model_local,
            model_top,
            test_loss_fn_local_top,
        ...
        if valid_cross_entropy < min_cross_entropy:
            ...
            torch.save(
                ...
                f"model_global_cross_entropy_{dataset_name}_{subject_id}.pth",
            )
            torch.save(
                model_local.state_dict(),
                f"model_local_cross_entropy_{dataset_name}_{subject_id}.pth",
            )
            torch.save(
                model_top.state_dict(),
                f"model_top_cross_entropy_{dataset_name}_{subject_id}.pth",
            )
    ...
    model_global.load_state_dict(
        torch.load(f"model_global_cross_entropy_{dataset_name}_{subject_id}.pth")
    )
    model_local.load_state_dict(
        torch.load(f"model_local_cross_entropy_{dataset_name}_{subject_id}.pth")
    )
    model_top.load_state_dict(
        torch.load(f"model_top_cross_entropy_{dataset_name}_{subject_id}.pth")
    )
    valid_total_loss, valid_cross_entropy, valid_acc = tcanet_net(
        model_global,
        model_local,
