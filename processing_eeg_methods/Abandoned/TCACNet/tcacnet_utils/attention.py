...
def extract_slices(inputs, batch_size, left_coord, channels, device):

    slices = []
    for i in range(batch_size):
        sliced = inputs[i].narrow(1, left_coord[i, 0], channels)
...
def extract_one_feature(inputs, indexes, feature_w, batch_size, channels, device):

    left_coord, idx_i, idx_j = left_coordinates(indexes, feature_w, device)
    slices = extract_slices(inputs, batch_size, left_coord, channels, device)
...
def extract_features(
    inputs, model_local, batch_size, k_indexes, feature_w, n_slices, channels, device
):

    k_src_idxs = []
    k_slices = []
    for i in range(n_slices):
        src_idxs, slices, left_coord = extract_one_feature(
            inputs, k_indexes[:, i], feature_w, batch_size, channels, device
        )
        k_src_idxs.append(src_idxs)
        k_slices.append(slices)
    concat_slices = torch.cat(k_slices, 0).to(device)
    assert concat_slices.shape == (batch_size * n_slices, 1, channels, 199)
...
def replace_features(global_features, local_features, replace_idxs, device):
    ...
    def _convert_to_1d_idxs(src_idxs):
        ...
        batch_idx_base = (
            torch.Tensor([i * batch_idx_len for i in range(batch_size)]).long()
        ).to(device)
    ...
    if flat_global_replaced.size() != flat_local_features.size():
        print(
            "Assertion error : flat_global_replaced.size()",
            flat_global_replaced.size(),
            " !=  flat_local_features.size()",
            flat_local_features.size(),
        )
...
def inference(
    ...
    with torch.enable_grad():
        conv2_weight = model_global.conv2.weight.reshape(-1, channels)
        ...
        two_22 = torch.full((channels,), 2.0)
        ...
        d_ent_d_features = torch.autograd.grad(
            entropy, global_features, grad_outputs=torch.ones(entropy.size()).to(device)
        )

    d_entropy_d_features = torch.cat(d_ent_d_features, 0)
    batch_size, feature_ch, feature_h, feature_w = global_features.size()
    top_k_indices = salient_indices(d_entropy_d_features, batch_size, n_slices)
    local_features, local_indexes = extract_features(
        inputs,
        model_local,
        batch_size,
        top_k_indices,
    ...
    merged, flat_global_replaced, flat_local_features = replace_features(
        global_features, local_features, local_indexes, device
    )
    raw_hint_loss = torch.sum(
        (flat_global_replaced - flat_local_features).pow(2), dim=0
    )
