...
from TCACNet.tcacnet_utils.network import (globalnetwork, localnetwork,
                                           topnetwork)
...
def EEGdata_loader(data, label, batch_size=16, batch_size_eval=16):
    x_train, x_test, x_valid, y_train, y_test, y_valid = train_test_val_split(
    ...
    train_loader = DataLoader(
        train_data, batch_size=batch_size, pin_memory=use_cuda, shuffle=True
    )
    valid_loader = DataLoader(
        valid_data, batch_size=batch_size_eval, pin_memory=use_cuda
    )
...
from TCACNet.tcacnet_utils.attention import inference
...
def train(
    ...
    for batch_idx, (inputs, target) in enumerate(train_loader):
        ...
        output_merged, hint_loss, channel_loss = inference(
            inputs,
            wpser,
            model_global,
            model_local,
    ...
    if epoch % 10 == 0:
        print(
            ...
            "".format(
                epoch,
                loss_local_and_top.item() + hint_loss.item(),
                loss_local_and_top.item(),
                hint_loss.item(),
                channel_loss.item(),
            ),
            end="",
    ...
    return (
        loss_local_and_top.item() + hint_loss.item() + channel_loss.item(),
        loss_local_and_top.item(),
    )
...
def tcanet_test(
    ...
    if epoch % 10 == 0 and verbose:
        print(
            ...
            "  Accuracy: {}/{} ({:.0f}%)\n".format(
                avg_test_loss + avg_hint_loss + avg_channel_loss,
                avg_test_loss,
                avg_hint_loss,
                avg_channel_loss,
                correct,
...
def call_tcanet(
    ...
    test_loader,
    channels,
    only_global_model: bool = True,
):  # only use global model
    n_epochs = 200
    ...
    if only_global_model:
        optimizer = optim.Adam(
            list(model_global.parameters()) + list(model_top.parameters()),
            lr=learning_rate,
        )
    else:
        optimizer = optim.Adam(
            list(model_global.parameters())
            + list(model_local.parameters())
            + list(model_top.parameters()),
            lr=learning_rate,
        )
    ...
    for ep in range(n_epochs):
        train_total_loss, train_cross_entropy = train(
            model_global,
            model_local,
            model_top,
            optimizer,
        ...
        valid_total_loss, valid_cross_entropy, valid_acc, preds_tcanet = tcanet_test(
            model_global,
            model_local,
            model_top,
            test_loss_fn_local_top,
        ...
        if valid_cross_entropy < min_cross_entropy:
            ...
            torch.save(
                ...
                f"{ROOT_VOTING_SYSTEM_PATH}/Results/model_global_cross_entropy_{dataset_name}_{subject_id}.pth",
            )
            torch.save(
                model_local.state_dict(),
                f"{ROOT_VOTING_SYSTEM_PATH}/Results/model_local_cross_entropy_{dataset_name}_{subject_id}.pth",
            )
            torch.save(
                model_top.state_dict(),
                f"{ROOT_VOTING_SYSTEM_PATH}/Results/model_top_cross_entropy_{dataset_name}_{subject_id}.pth",
            )
...
if __name__ == "__main__":
    ...
    model_global, model_local, model_top, acc = call_tcanet(
        dataset_name,
        subject_id,
        train_loader,
        valid_loader,
    ...
    for data_chosen, trial_chosen in zip(data_test, labels_test):
        trial_data = CustomDataset(
            [data_chosen], [0]
        )  # The label is dummy, it's just for the code to work
        use_cuda = False
        loader = DataLoader(trial_data, pin_memory=use_cuda)
        output_array_true = tcanet_online_pred(
            model_global,
            model_local,
            model_top,
            loader,
        ...
        output_array_false = tcanet_online_pred(
            model_global,
            model_local,
            model_top,
            loader,
