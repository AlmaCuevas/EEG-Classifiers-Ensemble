# I tried all reasonable options
# It simply doesn't work.
# To "predict" takes the mean values of X already knowing Y.
# I tried:
#  * giving X and y of train, but also X_test.
#     * so it can do the means on the train but use them on X_test
#  * giving X and a fake y (for example a 0 or a 1)
#     * to see how the Y affected the ‘prediction’
#  * giving fake values of the mean of X (0.5, 0.5) or (0.5, -0.5)

# If the prediction is by knowing the mean of X
# at least the version of having the mean of the train should work.
# Simply this method looks like pure cheating.
# The accuracies are fake, if you need to give them the answer
# for them to know the answer it is a scam.
...
class EEG_dataset(Dataset):
    def __init__(self, data, label_original, true_label=1):
        ...
        for i, label_i in enumerate(label_original):
        ...
        (
            ...
            self.labels_test,
            self.labels_val,
        ) = train_test_val_split(dataX=data, dataY=label, valid_flag=True)

        ### Emptying Ram
        for element in [data, label]:
            del element
        ### Copying arrays
        self.data_train, self.labels_train = (
            self.data_train.copy(),
            self.labels_train.copy(),
        )
        self.data_test, self.labels_test = (
            self.data_test.copy(),
            self.labels_test.copy(),
        )
...
class EEG_dataset_online(Dataset):
    def __init__(self, data):
        self.section = "test"
        self.data_test = data
        self.labels_test = [1]  # Dummy, just to not fail. It doesn't affect anything.
...
class CCSP(nn.Module):
    def __init__(
        ...
        wavelet_kernel=64,
        nn_layers=[4],
        feature_reduction_network=[4],
        nchans=62,
        n_CSP_subspace_vectors=1,
    ):
        ...
        ### Wawelet
        self.wavelet_padding = nn.ZeroPad2d(
            (int(wavelet_kernel / 2) - 1, int(wavelet_kernel / 2), 0, 0)
        )
        self.freq = torch.tensor(
            [
                [4 + (i + 1) * 36 / (self.wavelet_filters + 1)]
                for i in range(self.wavelet_filters)
            ],
            requires_grad=True,
            device=device,
        )
        self.fwhm = torch.tensor(
            [[0.1] for _ in range(self.wavelet_filters)],
            requires_grad=True,
            device=device,
        )
        self.coefficient = torch.tensor(
            [[4 * math.log(2)] for _ in range(self.wavelet_filters)],
            requires_grad=True,
            device=device,
        )
        ### Neural Network
    ...
    def plot_CSP(self, CSP_output, labels):
        ...
        for i in range(self.F):
            ...
            ax.plot(class1[:, 0], class1[:, -1], "or", alpha=0.6)
            ax.set_yticklabels([])
            ax.set_xticklabels([])
        plt.show()
    ...
    def _design_wavelet(self, kernLength, freq, fwhm, coefficient, fs=100):
        ...
        timevec = (
            timevec.repeat(self.wavelet_filters)
            .reshape(self.wavelet_filters, kernLength)
            .to(device)
        )
    ...
    def CNN_block(self, prev_chans, chan, last_block):
        if not last_block:
            return nn.Sequential(
                nn.ZeroPad2d(
                    (int(self.kernLength / 2) - 1, int(self.kernLength / 2), 0, 0)
                ),
                nn.Conv2d(
                    prev_chans, chan, (1, self.kernLength), padding=0, bias=False
                ),
                nn.Sigmoid(),
                nn.BatchNorm2d(chan),
            )
    ...
    def pred(self, X, y):
        ...
        Y, _ = self(X)
        MU = (
            torch.tensor([torch.mean(Y[y == 0]), torch.mean(Y[y == 1])])
            .reshape(-1, 1)
            .to(device)
        )
    ...
    def pred_online(self, X, y, X_test):
        training_state = self.training
        self.training = False
        Y, _ = self(X)
        Y_test, _ = self(X_test)
        MU = (
            torch.tensor([torch.mean(Y[y == 0]), torch.mean(Y[y == 1])])
            .reshape(-1, 1)
            .to(device)
        )
        yhat = torch.zeros(X_test.shape[0])
        for i, xi in enumerate(Y_test):
            dis0 = torch.sqrt(torch.dot(((xi - MU[0]).T), (xi - MU[0])))
            dis1 = torch.sqrt(torch.dot(((xi - MU[1]).T), (xi - MU[1])))
            if dis0 <= dis1:
                yhat[i] = 0
    ...
    def forward(self, x, visualization=False):
        ...
        if self.wavelet_filters:
            ...
            self.wavelet_weight = self._design_wavelet(
                self.wavelet_kernel, self.freq, self.fwhm, self.coefficient
            )
...
class CommonSpatialPattern(nn.Module):
    ...
    def fit(self, X, y):
        ...
        class0 = X[
            [True if (i == unique_values[0]).all() else False for i in y]
        ]  # (N_class 0 , F, NEc, Tp)
        class1 = X[
            [True if (i == unique_values[1]).all() else False for i in y]
        ]  # (N_class 1 , F, NEc, Tp)
        total_W = torch.tensor([])  # (F, 2*m , NEc)
        for i in range(self.F):
            class0_i = class0[:, i, ...]  # (N_class 1 , 1, NEc, Tp)
            class1_i = class1[:, i, ...]
            class0_i = class0_i - class0_i.mean(axis=2).reshape(
                class0_i.shape[0], class0_i.shape[1], 1
            )
            class1_i = class1_i - class1_i.mean(axis=2).reshape(
                class1_i.shape[0], class1_i.shape[1], 1
            )
            RH = 0
            for x in class1_i:
            ...
            W = torch.tensor(
                np.concatenate((sorted_u[:, : self.m], sorted_u[:, -self.m :]), axis=1)
            )  # (NEc, 2m)
...
class LinearDiscriminantAnalysis(nn.Module):
    ...
    def fit(self, X, y):
        ...
        unique_values = np.unique(y, axis=0)
        Xc1 = (
            X[[True if (i == unique_values[0]).all() else False for i in y]]
        ).T  # (N_class 0 , F, NEc, Tp)
        Xc2 = (
            X[[True if (i == unique_values[1]).all() else False for i in y]]
        ).T  # (N_class 1 , F, NEc, Tp)
...
def train(
    ...
    if verbose in [1, 2]:
        print(
            "Number of Trials:",
            dataloader.dataset.data_train.shape[0],
            "Batch Size:",
            batch_size,
            "Number of Batches:",
            n_batches,
    ...
    dataset.train()
    all_inputs, all_labels = torch.tensor(dataloader.dataset.data_train).float().to(
        device
    ), torch.tensor(dataloader.dataset.labels_train).to(device)
    ...
    for epoch in range(epochs):
        ...
        for batch, (inputs, labels) in enumerate(dataloader):
            ...
            inputs, labels = Variable(inputs).float().to(device), Variable(labels).to(
                device
            )
            ...
            for f in range(CSP_output_softmax.size(1)):
                CSP_loss += nn.BCELoss()(
                    CSP_output_softmax[:, f, ...].float(), BCElabels
                )
            ...
            loss = (
                loss_ratio * CSP_loss
                + (1 - loss_ratio)
                * criterion(outputs, labels.type(torch.LongTensor).to(device))
        ...
        with torch.no_grad():
            ...
            if batch_size < len(dataloader.dataset.data_train):
                if verbose in [3]:
                    ...
                    for batch, (test_inputs, test_labels) in enumerate(dataloader):
                        test_inputs, test_labels = Variable(test_inputs).float().to(
                            device
                        ), Variable(test_labels).to(device)
                        ...
                        test_loss = loss_ratio * CSP_loss + (
                            1 - loss_ratio
                        ) * criterion(
                            test_outputs, labels.type(torch.LongTensor).to(device)
                        )
                        predicted_labels = model.pred(test_inputs, test_labels).to(
                            device
                        )
                        ts_corrects += (predicted_labels == test_labels).sum().item()
                        ts_acc = (predicted_labels == test_labels).sum().item() / (
                            len(test_inputs)
                        )
                        print(
                            f"\033[Kaccuracy before updating CSP and LDA | @ Batch: {batch + 1} / | Acc = {ts_acc}",
                            end="\r",
                        )
            ...
            if validation:
                ...
                for batch, (test_inputs, test_labels) in enumerate(dataloader):
                    ...
                    CSP_loss = CSP_loss / CSP_output_softmax.size(1)
                    test_loss = loss_ratio * CSP_loss + (1 - loss_ratio) * criterion(
                        test_outputs, test_labels.type(torch.LongTensor).to(device)
                    )
                    ...
                    if verbose in [2]:
                        print(
                            f"\033[KNew Test Accuracy | @ Batch: {batch+1} / | Acc = {ts_acc}",
                            end="\r",
                        )
                total_ts_acc = ts_corrects / len(dataloader.dataset)
        ### TensorBoard
        ...
        if verbose in [2, 3]:
            ...
            print(
                f"\033[K Epoch: {epoch + 1}/ {epochs}: Train Acc=  {color(round(total_tr_acc * 100,2))}",
                end="\r",
            )
...
def evaluate(model, state, dataloader, loss_ratio, criterion, m, verbose=1):
    ...
    with torch.no_grad():
        model.eval()
        ts_corrects = 0
        test_inputs, test_labels = torch.tensor(
            dataloader.dataset.data_test
        ).float().to(device), torch.tensor(dataloader.dataset.labels_test).to(device)
        test_outputs, test_data = model(test_inputs)
        CSP_output_softmax = F.softmax(test_data["CSP output"], dim=-1)
        CSP_loss = 0
        BCElabels = output_format(test_labels, m)
    ...
    if verbose > 0:
        print(
            (
                f"\033[K ► Test Loss=  {test_loss.item():.4f}  Test Acc= {color(round(ts_acc * 100, 2))}"
            )
        )
    return test_loss, ts_acc


def evaluate_online(model, state, dataloader, train_inputs, train_labels):
    model.paste(state)
    with torch.no_grad():
        model.eval()
        test_inputs = torch.tensor(dataloader.dataset.data_test).float().to(device)
        predicted_labels = model.pred_online(
...
if __name__ == "__main__":
    ...
    for i in range(1, dataset_info["subjects"]):
        ...
        computer_root_path = (
            "/Users/almacuevas/work_projects/voting_system_platform/Datasets/"  # MAC
        )
        data_path = computer_root_path + dataset_foldername

        data, label = load_data_labels_based_on_dataset(
            dataset_info, subject, data_path
        )
        ...
        dataset = EEG_dataset(data_train, labels_train, true_label=true_label)
        ### Define Dataloader
        dataloader = DataLoader(
            dataset, batch_size=5300, num_workers=workers, pin_memory=True, shuffle=True
        )
        net_args = {
            "kernLength": 32,
            "timepoints": 250,
        ...
        test_loss, ts_acc = evaluate(
            net,
            best_model_state,
            dataloader,
            m=net_args["n_CSP_subspace_vectors"],
            criterion=MVLoss,
            loss_ratio=train_args["loss_ratio"],
            verbose=1,
        )
        ...
        print(
            "The Best Epoch based on accuracy:",
            np.argmax(history["train_accuracy"]) + 1,
        )
        test_loss, ts_acc = evaluate(
            net,
            best_model_state,
        ...
        train_inputs, train_labels = torch.tensor(
            dataloader.dataset.data_train
        ).float().to(device), torch.tensor(dataloader.dataset.labels_train).to(device)
        pred_list = []
        for data_chosen, trial_chosen in zip(data_test, labels_test):
            dataset = EEG_dataset_online(data_chosen)
            dataloader = DataLoader(
                dataset,
                batch_size=5300,
                num_workers=workers,
            ...
            answer = evaluate_online(
                net, best_model_state, dataloader, train_inputs, train_labels
            )
