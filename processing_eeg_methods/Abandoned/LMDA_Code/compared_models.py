...
class ShallowConvNet(nn.Module):
    ...
    def forward(self, x):
        ...
        features = torch.flatten(
            x, 1
        )  # 使用卷积网络代替全连接层进行分类, 因此需要返回x和卷积层个数
...
class EEGNet(nn.Module):
    def __init__(
        ...
        self.features = nn.Sequential(
            ...
            nn.BatchNorm2d(F1),
            nn.Conv2d(
                F1, F1, kernel_size=(chans, 1), groups=F1, bias=False
            ...
            nn.Conv2d(
                F1, F1, kernel_size=(1, 16), groups=F1, bias=False
            ),  # groups=F1 for depthWiseConv
            nn.BatchNorm2d(F1),
            nn.ELU(inplace=True),
            # nn.ReLU(),
            nn.Conv2d(
                F1, F2, kernel_size=(1, 1), groups=1, bias=False
            ),  # point-wise cnn
            nn.BatchNorm2d(F2),
            # nn.ReLU(),
            nn.ELU(inplace=True),
