...
from processing_methods.data_preprocess import (bandpass_cnt, mne_apply,
                                                preprocess4mi)
# tools for pytorch
...
def bci4_2a():  # 通用模型模块, 不需要更改, 放在main中方便调试;
    ...
    data_path = "/Users/rosit/Documents/MCC/voting_system_platform/processing_eeg_methods/LMDA_Code/BCICIV_2a_edf"
    ...
    train_cnt = mne_apply(
        lambda a: bandpass_cnt(
            a, low_cut_hz=4, high_cut_hz=38, filt_order=200, fs=250, zero_phase=False
        ),
        train_cnt,
    )
    ...
    if share_model_name == "LMDA":
        ...
        Net = ShallowConvNet(num_classes=mi_class, chans=channels, samples=samples).to(
            device
        )
