...
class Experiment(object):
    def __init__(
        ...
        class_weight[
            0
        ] *= imbalanced_class  # kaggle ERN, 0:1=3:7, 因此imbalanced_weight=3/7
        class_weight = torch.FloatTensor(class_weight).to(device)
        self.nllloss = nn.CrossEntropyLoss(weight=class_weight)
        self.datasets = OrderedDict(
            (("train", train_dl), ("valid", val_dl), ("test", test_dl))
        )
        if val_dl is None:
    ...
    def train_epoch(self):
        ...
        for i, (train_input, train_target) in enumerate(self.datasets["train"]):
            ...
            if self.p300_flag:  # AUC instead of acc
                ...
                batch_acc = np.equal(
                    predicted.cpu().detach().numpy(), train_label.cpu().detach().numpy()
                ).sum() / len(train_label)
        ...
        self.train_df = self.train_df[
            list(train_dicts_per_epoch.keys())
        ]  # 让epoch_df中的顺序和row_dict中的一致
    ...
    def monitor_epoch(self, datasets):
        ...
        self.val_df = self.val_df[
            list(result_dicts_per_monitor.keys())
        ]  # 让epoch_df中的顺序和row_dict中的一致
    ...
    def save_acc_loss_fig(self):
        ...
        plt.savefig(
            self.fig_path
            + "mean{:.3f}max{:.3f}.png".format(
                self.measure.last10_acc(), self.measure.max_acc()
            )
        )
