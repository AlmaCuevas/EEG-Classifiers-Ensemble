...
import torch
from compared_models import (EEGNet, MaxNormDefaultConstraint, ShallowConvNet,
                             weights_init)
from data_loaders import load_data_labels_based_on_dataset
# dataloader and preprocess
from data_preprocess import preprocess4mi
...
def data_and_model(dataset_name: str, valid_flag: bool = False):
    epochs, data, label = load_data_labels_based_on_dataset(
        dataset_info=dataset_name, subject_id=subject_id, data_path=data_path
    )
    data = preprocess4mi(
        data
    )  # This was for MI, try it. If it doesn't work, delete it.
    x_train, x_test, x_val, y_train, y_test, y_val = train_test_val_split(
        dataX=data, dataY=label, valid_flag=valid_flag
    )
    ...
    train_dl = DataLoader(
        train_loader, batch_size=batch_size, shuffle=True, drop_last=False, **kwargs
    )
    test_dl = DataLoader(
        test_loader, batch_size=batch_size, shuffle=False, drop_last=False, **kwargs
    )
    ...
    folder_path = "./%s/%s/" % (
        dataset_foldername,
        subject_id,
    )  # mkdir in current folder, and name it by target's num
    ...
    logging.info(
        "****************  %s for %s! ***************************", model_id, subject_id
    )

    if share_model_name == "LMDA":
        Net = LMDA(
            num_classes=model_para["num_classes"],
            chans=model_para["chans"],
            samples=data.shape[3],
            channel_depth1=model_para["channel_depth1"],
            channel_depth2=model_para["channel_depth2"],
            kernel=model_para["kernel"],
            depth=model_para["depth"],
            ave_depth=model_para["pool_depth"],
            avepool=model_para["avepool"],
        ).to(device)
...
if __name__ == "__main__":
    ...
    dataset_name = "aguilera_gamified"  # Only 3 things I should be able to change
    share_model_name = "LMDA"  # Only 3 things I should be able to change
    assert share_model_name in ["LMDA", "EEGNet", "ConvNet"]

    dataset_info = datasets_basic_infos[dataset_name]
    times = []
    for subject_id in range(1, 9):  # dataset_info['subjects']+1):
        # Folders and paths
        dataset_foldername = dataset_name + "_dataset"
        # computer_root_path = "/Users/almacuevas/work_projects/voting_system_platform/Datasets/" # MAC
        computer_root_path = "/Users/rosit/Documents/workprojects/bci_complete/voting_system_platform/Datasets/"  # OMEN
        ...
        model_para = {
            ...
            "channel_depth1": dataset_info["#_channels"]
            + 2,  # It is recommended that the number of convolutional layers in the time domain is more than the number of convolutional layers in the spatial domain.
            "channel_depth2": 9,
            "kernel": 75,
            "depth": 9,
            "pool_depth": 1,
        ...
        if share_model_name == "LMDA":
            ...
            code_num = "D{depth}_D{depth1}_D{depth2}_pool{pldp}".format(
                depth=model_para["depth"],
                depth1=model_para["channel_depth1"],
                depth2=model_para["channel_depth2"],
                pldp=model_para["avepool"] * model_para["avgpool_step1"],
            )
        else:
            code_num = ""
        ...
        kwargs = {
            "num_workers": 1,
            "pin_memory": True,
        }  # 因为pycharm开启了多进行运行main, num_works设置多个会报错
        # ================================================================================
