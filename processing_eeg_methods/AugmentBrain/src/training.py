...
def fit_model(
    ...
    network_hyperparameters_dict: dict,
    ...
    if aug_hyperparameters_dict["emd_static_augmentation"]:
        train_X, train_y = emd_static_augmentation(
            train_X,
            train_y,
    ...
    if aug_hyperparameters_dict["gan_static_augmentation"]:
        ...
        num_samples_per_label = int(
            len(train_X)
            / num_classes
            * aug_hyperparameters_dict["gan_augment_multiplier"]
        )
    ...
    train_generator = train_generator_with_aug(
        train_X, train_y, batch_size=batch_size, **aug_hyperparameters_dict
    )
    ...
    save_hyperparameters_dicts(
        aug_hyperparameters_dict, network_hyperparameters_dict, model_path
    )


def save_hyperparameters_dicts(
    aug_hyperparameters_dict, network_hyperparameters_dict, model_path
):
    ...
    model_function = network_hyperparameters_dict["network_to_train"]
    network_hyperparameters_dict["network_to_train"] = network_hyperparameters_dict[
        "network_to_train"
    ].__name__
...
def get_callback_lists(model_path, metric_to_monitor):
    callbacks_list = [
        ...
        keras.callbacks.ModelCheckpoint(
            ...
            + "/saved-models-{epoch:06d}-{val_accuracy:.5f}.h5",
            monitor=f"{metric_to_monitor}",
            save_best_only=True,
        ),
...
def kfold_cross_val(
    ...
    save_hyperparameters_dicts(
        aug_hyperparameters_dict, network_hyperparameters_dict, models_path
    )
    ...
    for train_indexes, test_indexes in strat_kfold.split(data_X, data_y):
        ...
        train_X, val_X, train_y, val_y = train_test_split(
            data_X[train_indexes],
            data_y[train_indexes],
            test_size=2 / 9,
...
def print_save_kfold_run_results(acc_per_fold, loss_per_fold, models_path):
    ...
    for i in range(0, len(acc_per_fold)):
        ...
        print(
            f"> Fold {i + 1} - Loss: {loss_per_fold[i]:.3f} - Accuracy: {acc_per_fold[i]:.3f}%"
        )
    print("------------------------------------------------------------------------")
    ...
    print("------------------------------------------------------------------------")
    with open(
        models_path / f"training_results_acc:{np.mean(acc_per_fold):.3f}", "w+"
    ) as file:
...
class Hyperparameters:
    ...
    def set_default_hyperparameters(self):
        ...
        aug_hyperparameters_dict["emd_sample_probability"] = (
            0  # For performance issues is recommended maintain
        )
...
def test_routine(
    ...
    network_hyperparameters_dict, aug_hyperparameters_dict = (
        hyperparameters.set_default_hyperparameters()
    )
    kfold_cross_val(
        data_X, data_y, network_hyperparameters_dict, aug_hyperparameters_dict
    )
...
def main():
    ...
    actual_valid_split_fraction = SPLITTING_PERCENTAGE.val / (
        100 - SPLITTING_PERCENTAGE.test
    )
    train_X, val_X, train_y, val_y = train_test_split(
        tmp_train_X,
        tmp_train_y,
        test_size=actual_valid_split_fraction,
        random_state=network_hyperparameters_dict["RANDOM_STATE"],
