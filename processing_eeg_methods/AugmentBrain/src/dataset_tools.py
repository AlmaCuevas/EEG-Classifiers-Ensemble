...
def split_data(
    starting_dir="../personal_dataset",
    splitting_percentage=(70, 20, 10),
    shuffle=True,
    coupling=False,
    division_factor=0,
):
    ...
    for action in ACTIONS:
        ...
        if untouched_per != 0:
            ...
            for sample in range(
                ...
                np.save(
                    file=os.path.join(tmp_dir, str(sample)), arr=action_data[sample]
                )
...
def load_all_raw_data(starting_dir: Path, channels=8, NUM_TIMESTAMP_PER_SAMPLE=250):
    ...
    filtered_actions = [
        action_dir
        for action_dir in starting_dir.iterdir()
        if action_dir.name in ACTIONS
    ]
    for index, actions_dir in enumerate(filtered_actions):
        if actions_dir.name in ACTIONS:
            for sample_path in actions_dir.iterdir():
                data_X = np.append(
                    data_X, np.expand_dims(np.load(str(sample_path)), axis=0), axis=0
                )
...
def visualize_save_data(
    data, title, visualize_data=True, file_path=None, save_data=False
):
...
def preprocess_raw_eeg(
    data, fs=250, lowcut=2.0, highcut=65.0, MAX_FREQ=60, power_hz=50, coi3order=3
):
    """
        ...
        for channel in range(len(data[0])):
            ...
            if coi3order != 0:
                DataFilter.perform_wavelet_denoising(
                    data[sample][channel], "coif3", coi3order
                )

            DataFilter.perform_bandpass(
                data[sample][channel],
                sampling_rate=fs,
                center_freq=int((lowcut + highcut) / 2),
...
def emd_static_augmentation(
    ...
    for class_index in classes_bar:
        num_class_augmented_sample = int(
            len(indices_list[class_index]) * augment_multiplier
        )
        samples_bar = tqdm(
            range(num_class_augmented_sample),
            leave=False,
            ...
            for imf_index in range(max_imft):
                ...
                for channel in range(num_channels):
                    ...
                    try:
                        ...
                        pass
            train_X = np.append(
                train_X, np.expand_dims(augmented_sample, axis=0), axis=0
            )
    ...
    gaussian_noise_matrix = np.random.normal(
        loc=0, scale=gaussian_noise_std, size=train_X.shape
    )
...
def add_noise_on_stft_sample_data(
    sample: np.ndarray, fs: float, window_size: int, gaussian_noise_std: float
):
    ...
    gaussian_noise_matrix = np.random.normal(
        loc=0, scale=gaussian_noise_std, size=Zxx.shape
    )
    perturbed_Zxx_magnitude = Zxx_magnitude + gaussian_noise_matrix

    perturbed_Zxx = perturbed_Zxx_magnitude * (
        np.cos(Zxx_angle) + 1j * np.sin(Zxx_angle)
    )
...
def train_generator_with_aug(
    train_X: np.ndarray,
    train_y: np.ndarray,
    batch_size: int,
    shuffle_channel_probability=0.0,
    mirror_channel_probability=0.0,
    stft_noise_sample_probability=0.0,
    shuffle_factor=0,
    gaussian_noise_std=0.0,
    gaussian_noise_stft_std=0.0,
    stft_window_size=20,
    emd_sample_probability=0.0,
    max_imft=6,
    **kwargs,
):
    """
    ...
    while True:
        ...
        for principal_sample_index in epoch_indexes:
            if np.random.random() < emd_sample_probability:
                ...
                for imf_index in range(max_imft):
                    current_imf_sample_index = np.random.choice(
                        indices_lists[class_index]
                    )
                    current_imf_sample = train_X[current_imf_sample_index]
                    for channel in range(num_channels):
                        channel_imf = emd.sift.sift(current_imf_sample[channel])
                ...
                class_index = index_to_class_dict[principal_sample_index]
                augmentation_sample_indexes = np.random.choice(
                    indices_lists[class_index], shuffle_factor
                )
                augmented_sample = np.zeros((1, train_X.shape[1], train_X.shape[2]))
                for channel in range(train_X.shape[1]):
                    if (
                        np.random.random() > shuffle_channel_probability
                        and np.random.random() > mirror_channel_probability
                    ):
                        augmented_sample[0, channel, :] = train_X[
                            principal_sample_index, channel, :
                        ]
                    elif (
                        np.random.random() < shuffle_channel_probability
                        and np.random.random() > mirror_channel_probability
                        ...
                        augmented_sample[0, channel, :] = train_X[
                            np.random.choice(augmentation_sample_indexes), channel, :
                        ]
                    elif (
                        np.random.random() > shuffle_channel_probability
                        and np.random.random() < mirror_channel_probability
                    ):
                        reverted_channel_sample = train_X[
                            principal_sample_index, channel, ::-1
                        ]
                        augmented_sample[0, channel, :] = reverted_channel_sample
                    else:
                        reverted_channel_sample = train_X[
                            np.random.choice(augmentation_sample_indexes), channel, ::-1
                        ]
                        augmented_sample[0, channel, :] = reverted_channel_sample

            if np.random.random() < stft_noise_sample_probability:
                augmented_sample = add_noise_on_stft_sample_data(
                    augmented_sample,
                    fs=BOARD_SAMPLING_RATE,
            ...
            if len(batch_train_X) == batch_size:
                gaussian_noise_matrix = np.random.normal(
                    loc=0, scale=gaussian_noise_std, size=batch_train_X.shape
                )
        ...
        if (
            len(batch_train_X) > 0
        ):  # prevent yielding empty batch when len(train_X) % batch_size == 0
            gaussian_noise_matrix = np.random.normal(
                loc=0, scale=gaussian_noise_std, size=batch_train_X.shape
            )
            batch_train_X = batch_train_X + gaussian_noise_matrix
...
class TrainSequenceWithAug(Sequence):
    ...
    def __init__(
        ...
        self.indices_lists = [
            np.where(train_y == i)[0] for i in range(self.num_classes)
        ]
    ...
    def __getitem__(self, idx):
        ...
        for sample_idx, sample in enumerate(batch_x):
            ...
            if np.random.random() < self.emd_sample_probability:
                ...
                for imf_index in range(self.max_imft):
                    current_imf_sample_index = np.random.choice(
                        self.indices_lists[class_index]
                    )
                ...
                augmentation_sample_indexes = np.random.choice(
                    self.indices_lists[class_index], self.shuffle_factor
                )
                # augmented_sample = np.zeros((1, self.train_X.shape[1], self.train_X.shape[2]))
                for channel in range(self.train_X.shape[1]):
                    if (
                        np.random.random() > self.shuffle_channel_probability
                        and np.random.random() > self.mirror_channel_probability
                    ):
                        pass
                    elif (
                        np.random.random() < self.shuffle_channel_probability
                        ...
                        batch_x[sample_idx, channel, :] = self.train_X[
                            np.random.choice(augmentation_sample_indexes), channel, :
                        ]
                    elif (
                        np.random.random() > self.shuffle_channel_probability
                        ...
                        reverted_channel_sample = self.train_X[
                            np.random.choice(augmentation_sample_indexes), channel, ::-1
                        ]
                        batch_x[sample_idx, channel, :] = reverted_channel_sample

            if np.random.random() < self.stft_noise_sample_probability:
                batch_x[sample_idx] = add_noise_on_stft_sample_data(
                    sample,
                    fs=BOARD_SAMPLING_RATE,
        ...
        gaussian_noise_matrix = np.random.normal(
            loc=0, scale=self.gaussian_noise_std, size=batch_x.shape
        )
    ...
    def on_epoch_end(self):
        ...
        self.train_y = self.train_y[epoch_indexes]
        self.indices_lists = [
            np.where(self.train_y == i)[0] for i in range(self.num_classes)
        ]
